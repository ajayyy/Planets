package app.ajay.planets.client.desktop.networking;
...
import app.ajay.planets.client.networking.ComClient;
...
public class WSClient implements ComClient {
	...
	private int myID;
	private WebSocketClientMessenger c;

	long startTime;

	// For Bidirectional Communication mode
	public WSClient(String ip, int port, WebSocketClientMessenger c) {
		this.port = port;
		connected = false;
	...
	public void connectClient(String ip) {
		if (!ip.isEmpty()) {
			...
			try {
				url = new URI("ws://" + ip + ":" + port); // We create the URI of the server. Use a port higher than 1024
			...
			wsc = new WebSocketClient(url, standard) {
				...
				public void onMessage(String message) {
					...
					// SERVER SEND MY CLIENT ID.
						...
						long serverPing = System.nanoTime() - startTime;
						c.onConnect(System.nanoTime() - serverPing / 2);
